...
@Inject
constructor(
    @LocalDataSource private val local: WordsDataSource,
    @RemoteDataSource private val remote: WordsDataSource
) : WordsRepository {
...
//        }
    ...
    private fun getRandomWord(letter: String): Word {
        var filteredWords = wordsCache?.filter { word ->
            word.value.first().toString() == letter && word.value.length <= 6
        }
        if (filteredWords != null && filteredWords.isEmpty()) {
            filteredWords =
                wordsCache?.filter { word -> word.value.contains(letter) && word.value.length <= 6 }
        }
    ...
    override fun addImageToWord(word: Word, image: Image): Completable {
        return remote.addImageToWord(word, image)
    }

    override fun getWordById(wordId: String): Single<Word> {
        return remote.getWordById(wordId)
    }
}